...
var KNodeModel = dbConnection.model('KNode', (<any>global).db.kNode.Schema);
        ...
        case 'in_map_of_type_and_content_data':
            ...
            if(isPrmNull(id) || isPrmNull(id2) || isPrmNull(id3)  || isPrmNull(id4)){
              ...
              //  searchObj1 = { id3 : parseInt(id4) }
                // KNodeModel.find(searchObj, found);
                let searchObj = { $and: [{ mapId: id }, { type: id2 }, searchObj1 ] };
...
export function findMaxVal(name, type, mapId, res, callback=null){
  console.log('findMaxVal::params: ',name, type, mapId, res, callback);
  exports._index(name, type, mapId, null, 'max_val_type_map', res, callback);
}

export function needHumanId(node){
  switch (node.type) {
...
export function _create(data, res, callback, second?, third?) {
...
export function _update(data, id, actionType, callback) {
...
export function _destroyOne(searchParam, callback) {
...
export function _destroyInMap(searchParam, callback) {
...
export function _destroyByModificationSource(searchParam, callback) {
    console.log("[modules/kNode.js:destroy] deleting nodes in map %s", searchParam);
    KNodeModel.remove({ 'mapId': searchParam }, function(err) {
        if (err) {
            console.log("[modules/kNode.js:destroy] error:" + err);
...
//TODO: we must also filter by `mapId` but so far we are sending only 2 parameters!
export function _destroyByTypenUser(type, iAmId, callback) {
...
export function _destroyByTypeInMap(type, mapId, callback) {
