...
export class KNode extends VO {
  ...
  static TYPE_MY_COLABO_FLOW_STATE: string = "colaboflow.my.state";
  ...
  static nodeFactory(obj: any): KNode {
    //TODO:remove - kept only for legacy of the old code
  ...
  isIbis(): boolean {
    return (
      this.type == KNode.TYPE_IBIS_QUESTION ||
      this.type == KNode.TYPE_IBIS_IDEA ||
      this.type == KNode.TYPE_IBIS_ARGUMENT ||
      this.type == KNode.TYPE_IBIS_COMMENT
    );
  }
  ...
  toServerCopy(): any {
    ...
    for (id in this) {
      ...
      if (this[id] !== undefined) {
        //JSON.parse breaks at "undefined"
