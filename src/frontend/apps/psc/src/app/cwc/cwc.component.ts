...
export class CwcComponent implements OnInit {
  ...
  onSubmit() {
    ...
    this.openDialog(
      1,
      new DialogData("Submitting", "please wait ...", "Cancel"),
      { disableClose: true }
    );

    this.cwcs = new CWCData([
      this.form.value.cwcVision1,
      this.form.value.cwcVision2,
      this.form.value.cwcVision3
    ]);
  ...
  cwcsSaved(): void {
    ...
    this.openDialog(
      ...
      new DialogData(
        "Submitted",
        "Thank you for your CWC visions. You've finished this phase",
        "OK"
      )
    );
  }
