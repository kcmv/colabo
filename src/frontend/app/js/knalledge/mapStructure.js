(function() { // This prevents problems when concatenating scripts that aren't strict.
	...
	*/
	...
	MapStructure.prototype.setAncestorsVisibile = function(node) {
		var ancestors = this.getAllAncestorsPaths(node);
		for (var j in ancestors) {
			ancestors[j].presentation.visibleAsAncestor = true;
		}
		// console.log('setVisibility - getAllAncestorsPaths: ' + getNodesNames(ancestors));
	...
	MapStructure.prototype.openAncestors = function(node) {
		...
		var ancestors = this.getAllAncestorsPaths(node);
		for (var j in ancestors) {
			if (ancestors[j] != node) { // becasue getAllAncestorsPaths returns the `node` too
	...
	*/
	MapStructure.prototype.getAllAncestorsPaths = function(node) {
		...
		var ancestorsNew = {};
		for (var i = 0; i < parents.length; i++) {
			ancestorsNew = this.getAllAncestorsPaths(parents[i]);
			for (var ancestors_i in ancestorsNew) {
				ancestors[ancestors_i] = ancestorsNew[ancestors_i];
				//IMPORTANT: by this algorythm the same ancestor can be found and added if he is on multiple paths
				//(by using hash-object we override it, but by using array we should duplicate it)
			}
		}
		return ancestors;
	}

	MapStructure.prototype.isDescendantInDistance = function(ancestor, descendant, distance) {
		distance = typeof distance !== 'undefined' ? distance : -1;
		var currentDistanceFromDescendant = 0;
		...
		do{
			var parents = this.getParentNodes(descendant);
			if(parents.length > 1){
				throw new Error("function 'MapStructure.prototype.isDescendantInDistance' is designed to work with trees and not with graphs. But now a node with more than one parent is found!");
			}
			if(parents.length === 0){
				return false;
			}
			if (parents[0] === ancestor) {
				return true;
			}
			descendant = parents[0];
			currentDistanceFromDescendant++;
		}while (distance === -1 || currentDistanceFromDescendant < distance);
		return false;
	};
	...
	MapStructure.prototype.isOnAncestorsPath = function(nodDescendant, nodeAncestor) {
		var ancestors = this.getAllAncestorsPaths(nodDescendant);
	...
	*/
	...
	MapStructure.prototype.deleteNode = function(vnode, callback) {
		...
		var that = this;
		var result = this.mapService.deleteNode(vnode.kNode).$promise.then(function(delResult){
