(function() { // This prevents problems when concatenating scripts that aren't strict.
    ...
    */
     ...
     * @param  {knalledge.VKNode} d - node that we are looking related entities for
     * @return {Array.<D3 node>}
     */
    MapVisualization.prototype.getNodeRelatedD3Entities = function(d) {
        var nodeRelatedD3Entities = [];

        // halo
        if (this.halo &&
            (d === this.mapStructure.getSelectedNode())) {
            var haloView = this.halo.getHaloView();
            nodeRelatedD3Entities.push(haloView);
        }

        return nodeRelatedD3Entities;
    };

		/**
     * Returns all D3 entities that are related to the provided node
     * (this for example can be the D3 node representation of halo around the node)
     * @function positionNodeRelatedEntities
     * @memberof knalledge.MapVisualization
     * @param  {knalledge.VKNode} d - node that we are looking related entities for
     * @return {Array.<D3 node>}
     */
    MapVisualization.prototype.positionNodeRelatedEntities = function(d) {
        // var nodeRelatedD3Entities = [];
        // this.nodeFocus(d);
        // return;
        ...
        if (this.halo &&
            (d === this.mapStructure.getSelectedNode())) {
            // var haloView = this.halo.getHaloView();
            var dDom = this.getDomFromDatum(d);
            this.halo.updatePosition(dDom.node());
        }
    ...
    */
    MapVisualization.prototype.positionToDatum = function(datum) {
      if(datum === null || typeof datum === 'undefined'){
        ...
        var position = {
            x: divMapJQ.scrollLeft(),
            y: divMapJQ.scrollTop()
        };
        var target = {
            x: x,
            y: y
