...
remote_zip_path='/var/www/knalledge_frontend/prod/'$zipname
# sftp -v -oIdentityFile=path mprinc@knalledge.org <<EOF
sftp -v mprinc@knalledge.org <<EOF
put $local_zip_path $remote_zip_path
EOF

ssh -v mprinc@knalledge.org <<EOF
...
# remote_css_path='/var/www/knalledge_frontend/prod/css/'
