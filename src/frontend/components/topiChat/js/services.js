...
topiChatServices.provider('TopiChatQueue', {
	//TopiChatQueue.execute({data: kNode, callback:callback, resource_type:resource.RESOURCE_TYPE, method: "create", processing: {"RESOLVE":resolve, "REJECT":reject, "EXECUTE": resource.execute, "CHECK": resource.check}});
	// privateData: "privatno",
	$get: [/*'$q', '$rootScope', '$window',*/ function(/*$q, $rootScope, $window*/) {
		// var that = this;
		var provider = {
			queue: [],
			//OLD: not used any more linkToServices: {}, //TopiChatQueue.link(resource.RESOURCE_TYPE, {"EXECUTE": resource.execute, "CHECK": resource.check});
...
topiChatServices.factory('KnalledgeNodeService', ['$resource', '$q', 'ENV', 'TopiChatQueue', function($resource, $q, ENV, TopiChatQueue){
	console.log("[topiChatServices] server backend: %s", ENV.server.backend);
	...
	var resource = $resource(url, {}, {
		...
		getPlain: {method:'GET', params:{type:'one', searchParam:''}, isArray:false,
			...
			if(ENV.server.parseResponse){
				...
				console.log("[topiChatServices] accessId: %s", serverResponse.accessId);
	...
	resource.update = function(kNode, callback)
	{
		...
		if(QUEUE && false){
			TopiChatQueue.execute({data: kNode, callback:callback, resource_type:resource.RESOURCE_TYPE, method: "create"});
	...
	resource.execute = function(request){ //example:: request = {data: kNode, callback:callback, resource_type:resource.RESOURCE_TYPE, method: "create", processing: {"RESOLVE":resolve, "REJECT":reject, "EXECUTE": resource.execute, "CHECK": resource.check}};
		...
		case 'create':
			...
			var node = resource.createPlain({}, kNodeForServer, function(nodeFromServer){
				...
				TopiChatQueue.executed(request);
			});
	...
	//TopiChatQueue.link(resource.RESOURCE_TYPE, {"EXECUTE": resource.execute, "CHECK": resource.check});

	return resource;
	
}]);

topiChatServices.factory('KnalledgeEdgeService', ['$resource', '$q', 'ENV', 'TopiChatQueue', function($resource, $q, ENV, TopiChatQueue){
	...
	resource.create = function(kEdge, callback)
	{
		...
		if(QUEUE){
			...
			kEdge.$promise = $q(function(resolve, reject) {
				TopiChatQueue.execute({data: kEdge, callback:callback, resource_type:resource.RESOURCE_TYPE, method: "create", processing: {"RESOLVE":resolve, "REJECT":reject, "EXECUTE": resource.execute, "CHECK": resource.check}});
			});
		}
		else{
...
topiChatServices.provider('TopiChatVOsService', {
	// privateData: "privatno",
		...
		var provider = {
			...
			createNode: function(kNode, kNodeType) {
				...
				var nodeCreated = function(nodeFromServer) {
					console.log("[TopiChatVOsService] nodeCreated");// + JSON.stringify(nodeFromServer));
					// var edgeUpdatedNodeRef = function(edgeFromServer){
					// 	console.log("[TopiChatVOsService] edgeUpdatedNodeRef" + JSON.stringify(edgeFromServer));
					// };
					...
					for(var i in this.edgesById){
						...
						//TODO: check should we do here something, after TopiChatQueue logic is used etc:
				...
				console.log("[TopiChatVOsService] createNode");
	...
	//		newNode.kNode.$promise.then(function(kNodeFromServer){ // TODO: we should remove this promise when we implement TopiChatQueue that will solve these kind of dependencies
	//			console.log("KeyboardJS.on('tab': in promised fn after createNode");
			...
			createEdge: function(kEdge, callback) {
				var edgeCreated = function(edgeFromServer) {
					console.log("[TopiChatVOsService::createEdge] edgeCreated" + JSON.stringify(edgeFromServer));
			...
			createEdgeBetweenNodes: function(sourceNode, targetNode, kEdge, kEdgeType, callback) {
				console.log("[TopiChatVOsService] createEdge");
				// var maxId = -1;
				// for(var i in this.edgesById){
			...
			loadData: function(map){
				...
				// 	$window.alert("Error loading topiChat: %s", fail);
				// };
				
				var nodesEdgesReceived = function(){
					console.log("[TopiChatVOsService::loadData] nodesEdgesReceived");
					var i;
...
topiChatServices.factory('TopiChatService', ['$resource', '$q', 'ENV', 'TopiChatQueue', function($resource, $q, ENV, TopiChatQueue){
	console.log("[topiChatServices] server backend: %s", ENV.server.backend);
	// creationId is parameter that will be replaced with real value during the service call from controller
	...
	var resource = $resource(url, {}, {
		...
		getPlain: {method:'GET', params:{type:'one', searchParam:''}, isArray:false,
			...
			if(ENV.server.parseResponse){
				serverResponseNonParsed = removeJsonProtected(ENV, serverResponseNonParsed);
				serverResponse = JSON.parse(serverResponseNonParsed);
				// console.log("[TopiChatService] serverResponse: %s", JSON.stringify(serverResponse));
				console.log("[topiChatServices] accessId: %s", serverResponse.accessId);
				/* there is no use of transforming it to VO here, because it is transformed back to Resource by this method, so we do it in wrapper func that calls this one:
		...
		queryPlain: {method:'GET', params:{type:'', searchParam:''}, isArray:true, 
			...
			if(ENV.server.parseResponse){
				...
				serverResponse = JSON.parse(serverResponseNonParsed);
				console.log("[TopiChatService] serverResponse: %s", JSON.stringify(serverResponse));
				console.log("[TopiChatService] accessId: %s", serverResponse.accessId);
				/* there is no use of transforming it to VO here, because it is transformed back to Resource by this method, so we do it in wrapper func that calls this one:
				var data = serverResponse.data;
				var VOs = [];
				for(var datumId in serverResponse.data){
				...
				//console.log("[TopiChatService] data: %s", JSON.stringify(data));
				return VOs;
				*/
				return serverResponse.data;
			}else{
		...
		createPlain: {method:'POST', params:{}/*{type:'', searchParam: '', extension:""}*/, 
			...
			if(ENV.server.parseResponse){
				...
				serverResponse = JSON.parse(serverResponseNonParsed);
				//console.log("[TopiChatService] serverResponse: %s", JSON.stringify(serverResponse));
				console.log("[ng-TopiChatService::createPlain] accessId: %s", serverResponse.accessId);
				var data = serverResponse.data;
				console.log("ng-[TopiChatService::createPlain] data: %s", JSON.stringify(data));
				return data;
			}else{
				//console.log("ENV.server.parseResponse = false");
				serverResponseNonParsed = removeJsonProtected(ENV, serverResponseNonParsed);
		...
		updatePlain: {method:'PUT', params:{type:'one', searchParam:''},
			transformResponse: function(serverResponseNonParsed/*, headersGetter*/){
				...
				if(ENV.server.parseResponse){
					...
					serverResponse = JSON.parse(serverResponseNonParsed);
					//console.log("[TopiChatService] serverResponse: %s", JSON.stringify(serverResponse));
					console.log("[TopiChatService:create] accessId: %s", serverResponse.accessId);
					var data = serverResponse.data;
					return data;
				}else{
					serverResponseNonParsed = removeJsonProtected(ENV, serverResponseNonParsed);
	...
	resource.create = function(kMap, callback)
	{
		...
		if(QUEUE){
			...
			kMap.$promise = $q(function(resolve, reject) {
				TopiChatQueue.execute({data: kMap, callback:callback, resource_type:resource.RESOURCE_TYPE, method: "create", processing: {"RESOLVE":resolve, "REJECT":reject, "EXECUTE": resource.execute, "CHECK": resource.check}});
			});
		}
		else{
	...
	resource.update = function(kMap, callback)
	{
		...
		if(QUEUE && false){
			TopiChatQueue.execute({data: kMap, callback:callback, resource_type:resource.RESOURCE_TYPE, method: "create"});
...
topiChatServices.factory('SyncingService', ['$resource', '$q', 'ENV', 'TopiChatQueue', 'TopiChatVOsService',
	function($resource, $q, ENV, TopiChatQueue, TopiChatVOsService){
	console.log("[topiChatServices] server backend: %s", ENV.server.backend);
	// creationId is parameter that will be replaced with real value during the service call from controller
	...
	resource.getChangesFromServer = function(callback){
		...
		var updates = this.getPlain({type:'in_map_after', searchParam: TopiChatVOsService.mapId, searchParam2:this.lastUpdate}, function(updatesFromServer){
...
topiChatServices.provider('IbisTypesService', {
	// privateData: "privatno",
	$get: [/*'$q', 'ENV', '$rootScope', */
	function(/*$q , ENV, $rootScope*/) {
		var items = [
