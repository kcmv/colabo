...
export class PresentationServices {
            ...
            nodeHtmlUpdate: function(nodeHtmlUpdate){
              ...
              nodeHtmlUpdate.select(".node_presentation")
                ...
                // toggle node presence in presentation
            		.on("click", function(d){
            			console.log('presentation clicked for node ',d.name);
    ...
    */
    constructor(
      ...
      var that:PresentationServices = this;
    ...
    _getPresentationsNode(){
      ...
      if (this.mapBuilder){
        presentationsNode = this.mapBuilder._getNodesByType(PRESENTATIONS_NODE_TYPE);
      }
    ...
    _getPresentationNode(){
      ...
      if(this.mapBuilder){
        presentationNodes = this.mapBuilder._getNodesByType(PRESENTATION_NODE_TYPE);
        for(var pI=0; pI<presentationNodes.length; pI++){
          presentationNode = presentationNodes[pI];
          var sum = 0;
          if(presentationNode.dataContent && presentationNode.dataContent.ibis && presentationNode.dataContent.ibis.votes){
            for(var vote in presentationNode.dataContent.ibis.votes){
              sum+=presentationNode.dataContent.ibis.votes[vote];
            }
          }
          if(sum > sumMax){
    ...
    isNodeInSlides(vkNode){
      if(vkNode && this.mapStructure){
        ...
        var edges = this.mapStructure.getEdgesBetweenNodes(presentationNode, vkNode);
    ...
    addNodeToSlides(vkNode, slidePosition, callback?){
      var that:PresentationServices = this;
    ...
    removeNodeFromSlides(vkNode, callback?){
      ...
      if(!vkNode || !this.mapStructure || !this.isNodeInSlides(vkNode)){
        ...
        if(!presentationNode){
          ...
          var edges = this.mapStructure.getEdgesBetweenNodes(presentationNode, vkNode, SLIDE_EDGE_TYPE);
          if(edges.length <= 0){
            callback(null);
          }else{
    ...
    _createPresentationsNode (callback?:Function){
      ...
      presentationsVKNode = presentationsKNode;
    ...
    _createPresentationNode (callback?:Function){
      ...
      presentationVKNode = presentationKNode;
    ...
    createPresentation(){
      let that:PresentationServices = this;
    ...
    _sortEdgesByWeight(kEdgeA: KEdge, kEdgeB: KEdge):number{
      // ensure that numbers are always placed first
      if(typeof kEdgeA.value !== 'number') return 1;
      if(typeof kEdgeB.value !== 'number') return -1;

      return kEdgeA.value - kEdgeB.value;
    }
    ...
    */
    ...
    getSlideNodes(){
      ...
      if(this.mapBuilder){
        let presentationNode = this._getPresentationNode();
        if(presentationNode){
          let kEdges:KEdge[] = this.mapBuilder._getChildrenEdgesForNodeId(presentationNode._id, SLIDE_EDGE_TYPE);
          kEdges.sort(this._sortEdgesByWeight);
          for(let i=0; i<kEdges.length; i++){
            let kEdge:KEdge = kEdges[i];
            slideNodes.push(this.mapBuilder._getNodeFromId(kEdge.targetId));
          }
        }
      }
      return slideNodes;
    ...
    generatePresentationSlides(slidesDom, callback){
      ...
      for(var sI=0; sI<slideNodes.length; sI++){
        ...
        var slideContent =
          "# " + slideVKNode.name + "\r\n\r\n";
        if(slideVKNode.dataContent && slideVKNode.dataContent.property &&
          slideVKNode.dataContent.propertyType === 'text/markdown'){
          slideContent += slideVKNode.dataContent.property;
        }
        // vkNode.dataContent.propertyType
    ...
    getPresentationSlidesAsMarkdown(){
      ...
      for(var sI=0; sI<slideNodes.length; sI++){
        ...
        if(slideVKNode.dataContent && slideVKNode.dataContent.property &&
          slideVKNode.dataContent.propertyType === 'text/markdown'){
          var innerContent = slideVKNode.dataContent.property;
    ...
    getSlidePositionForSlideNode(positionedSlideNode):number{
      ...
      for(let s=0; s<slides.length; s++){
        ...
        positionedSlideId++;
        var contentType = slide.dataContent.propertyType;
        var content = slide.dataContent.property;
