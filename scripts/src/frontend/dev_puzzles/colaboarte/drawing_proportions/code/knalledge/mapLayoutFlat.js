...
var MapLayoutFlat =  knalledge.MapLayoutFlat = function(structure, configNodes, configTree, clientApi, knalledgeState){
...
MapLayoutFlat.prototype.getChildren = function(d){ //TODO: improve probably, not to compute array each time, but to update it upon changes
	var children = [];
...
MapLayoutFlat.prototype.init = function(mapSize, scales){
...
MapLayoutFlat.prototype.diagonal = function(that){
...
MapLayoutFlat.prototype.getAllNodesHtml = function(){
	return this.dom.divMapHtml.selectAll("div.node_html");
};

// Returns view representation (dom) from datum d
MapLayoutFlat.prototype.getDomFromDatum = function(d) {
...
MapLayoutFlat.prototype.clickNode = function(d, dom, commingFromAngular) {
	// select clicked
...
MapLayoutFlat.prototype.clickDoubleNode = function(d) {
	this.structure.toggle(d);
	this.clientApi.update(d);
};

// react on label click.
MapLayoutFlat.prototype.clickLinkLabel = function() {
...
MapLayoutFlat.prototype.viewspecChanged = function(target){
 ...
 */
MapLayoutFlat.prototype.generateTree = function(source){
	var that = this;
	if(this.nodes){
...
MapLayoutFlat.prototype.printTree = function(nodes) {
...
MapLayoutFlat.prototype.MoveNodesToPositiveSpace = function(nodes) {
	var minX = 0, maxX = 0, minY = 0, maxY = 0;
	var node;
	for(var i in nodes){
		node = nodes[i];
...
MapLayoutFlat.prototype.getHtmlNodePosition = function(d) {
	var x = null;
