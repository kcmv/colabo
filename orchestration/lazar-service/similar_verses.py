...
def get_ideas(q = '', mid=''):
    ...
    if len(q)==24 and ' ' not in q and set(q) - set('1234567890abcdef') == set([]):
        id_ = q.strip()
    collection = db['knodes']
    if not q:docs = collection.find({'type':"rima.user.dream", "mapId" : ObjectId(mid)})
    elif id_:docs = collection.find({'iAmId': ObjectId(q), 'type':"rima.user.dream", "mapId" : ObjectId(mid)})
    else:docs = collection.find({'$text': {'$search': q}, 'type':"rima.user.dream", "mapId" : ObjectId(mid)}, {'score': {'$meta': "textScore"}}).sort({'score':{'$meta':"textScore"}})
    for doc in docs:
        if doc['name']:
...
server = ThreadedServer(MyService, hostname='localhost', port = 12345, auto_register=None)
